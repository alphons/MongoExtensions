...
"countdocuments" };
	...
	{
		ColorOutput(Serializer.ToJson(0, val, new JsonSerializerOptions() { WriteIndented = true }) + Environment.NewLine);
	}
	...
	private static async Task ExecParsedAsync(FunctionArguments parsed)
	{
		...
		switch (parsed.Name.ToLower())
		{
			...
			case "color":
				...
				Serializer.Colorize = args == "on" || args == "1" || args == "true" || args == "yes";
				break;
			case "cls":
				Console.Clear();
				break;
